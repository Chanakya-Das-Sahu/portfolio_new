...
const ImageCarousel = ({ images, id, onImageChange }) => {
  ...
  useEffect(() => {
    ...
    const dots = document.querySelectorAll(`.${id}-dot-indicator`);
    ...
    return () => carousel.removeEventListener('scroll', updateActiveDot);
  }, [images.length, id, onImageChange]);
    ...
    <div className="relative h-full">
      ...
      >
        {images.map((image, index) => (
          <div key={index} className="flex-shrink-0 w-full h-full snap-start">
            <OptimizedImage
              src={`${id === 'khush' ? '/khush/' : '/images/'}${image}`}
      ...
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-1.5">
        {images.map((_, index) => (
          <button
            key={index}
            className={`w-2 h-2 rounded-full bg-white transition-all duration-300 ${id}-dot-indicator ${index === 0 ? 'opacity-100' : 'opacity-50'}`}
...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    navItems.forEach((item) => {
      ...
      if (element) {
        ...
        sectionRefs.current[item] = element;
      }
    });
  ...
  if (!mounted) {
    // Return a minimal skeleton to prevent layout shift
            ...
            <div className="grid md:grid-cols-3 gap-8">
              ...
              >
                <div className="h-64 relative overflow-hidden">
                  <ImageCarousel images={khush} id="khush" />
                </div>
                <div className="p-4 bg-white">
              ...
              >
                <div className="h-64 relative overflow-hidden">
                  <ImageCarousel images={kvImages} id="kv" />
                </div>
                <div className="p-4 bg-white">
